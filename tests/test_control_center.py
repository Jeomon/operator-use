...
async def test_enable_browser_use_calls_agent(tmp_path):
    ...
    with patch("operator_use.tools.control_center.CONFIG_PATH", cfg_file):
...
async def test_audit_log_emitted_on_plugin_change(tmp_path, caplog):
    ...
    with patch("operator_use.tools.control_center.CONFIG_PATH", cfg_file):
        with caplog.at_level(
            logging.WARNING, logger="operator_use.tools.control_center"
        ):
            await _call_cc(
                browser_use=True,
                _agent=mock_agent,
...
async def test_returns_error_when_config_missing(tmp_path):
    ...
    with patch("operator_use.tools.control_center.CONFIG_PATH", missing):
        result = await _call_cc(browser_use=True)
