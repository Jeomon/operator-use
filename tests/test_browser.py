...
# ---------------------------------------------------------------------------
# browser_task — reuse / session logic
# ---------------------------------------------------------------------------

def _make_mock_browser(use_system_profile=False, connected=True):
    b = MagicMock()
    b._client = MagicMock() if connected else None
    b.config = BrowserConfig(use_system_profile=use_system_profile)
    b.ensure_open = AsyncMock()
    b.close = AsyncMock()
    b._get_current_session_id = MagicMock(return_value=None)
    return b


@pytest.mark.asyncio
async def test_browser_task_reuses_existing_browser():
    from operator_use.web.subagent import browser_task
    from operator_use.providers.events import LLMEvent, LLMEventType

    mock_browser = _make_mock_browser(use_system_profile=False, connected=True)
    mock_llm = MagicMock()
    mock_llm.ainvoke = AsyncMock(return_value=LLMEvent(type=LLMEventType.TEXT, content='done'))

    result = await browser_task.function(
        task='test task',
        keep_open=True,
        _llm=mock_llm,
...
@pytest.mark.asyncio
async def test_browser_task_creates_new_browser_on_session_mismatch():
    """If task wants user session but existing browser is clean, create a new one."""
    from operator_use.web.subagent import browser_task
    from operator_use.providers.events import LLMEvent, LLMEventType
    ...
    def mock_browser_init(config=None):
        b = _make_mock_browser(use_system_profile=getattr(config, 'use_system_profile', False), connected=False)
        created_configs.append(config)
        return b

    with patch('operator_use.web.browser.service.Browser', side_effect=mock_browser_init):
        await browser_task.function(
            task='check my email',
            keep_open=False,
    ...
    assert created_configs[0].use_system_profile is True

@pytest.mark.asyncio
async def test_browser_task_use_user_session_false_uses_clean_profile():
    from operator_use.web.subagent import browser_task
    from operator_use.providers.events import LLMEvent, LLMEventType
    ...
    def mock_browser_init(config=None):
        b = _make_mock_browser(connected=False)
        created_configs.append(config)
        return b

    with patch('operator_use.web.browser.service.Browser', side_effect=mock_browser_init):
        await browser_task.function(task='search something', use_user_session=False, _llm=mock_llm)

    assert created_configs[0].use_system_profile is False
